...
from .. import utils
from .g_utils import L10NResult
from .g_domain import DomainGenerationProtocol
...
def _front_matter(token: Token, env: MutableMapping) -> L10NResult:
    domain_g: DomainGenerationProtocol = env['domain_generation']
    fm_result = domain_g.render_front_matter(token.content, token.markup)
    return fm_result


def _link_ref(env: MutableMapping, md_ctx: _MdCtx, content_result: L10NResult):
...
class RendererMarkdownL10N(RendererProtocol):
    ...
    def render(
        ...
        :param env: containing 'domain_generation' a `DomainGenerationProtocol` object
